...
**As-implemented (Phase 1):** Reconciled to match the codebase on 2026-05-19 after Phase 1 implementation.
...
| Layer | Phase 1 (implemented) | Phase 2 (planned — Commit 2) |
|-------|----------------------|--------------------------------|
| Dedupe | Inline in `IngestShipmentEventService` via `existsByPartnerAndEventId` | `DedupeStrategy` + `DedupeStrategyResolver` + yaml config |
| Config | `DomainConfiguration` (`StateProjector` bean only) | `PartnerConfigProperties` |
| Validation | `eventId` **@NotBlank** on `IngestShipmentEventRequest` | Optional `eventId` for natural-key partner |
...
**Phase 1 does not contain:** `DedupeStrategy`, `NaturalKeyDedupeStrategy`, `PartnerConfigProperties`, `DedupeStrategyResolver`, `EventIdDedupeHandler` (removed from earlier draft — dedupe is inline).

---

## Phase 2 — Planned additions (Commit 2)

Unchanged intent — extract dedupe to strategy pattern and add natural-key UK in Liquibase `003`.

```mermaid
classDiagram
    class DedupeStrategy {
        <<interface>>
        +supports(partner) boolean
...
Refactor: move current `existsByPartnerAndEventId` + duplicate row logic into `EventIdDedupeStrategy`; add `NaturalKeyDedupeStrategy` for `acme`.
...
| `ChangeRequestIntegrationTest` | **2** | Walkthrough steps 11–12 |
...
|------|------------------------|------|
| Dedupe | Inline in `IngestShipmentEventService` | ✓ No strategy classes |
...
*Phase 2 diagram remains the planned delta for Commit 2.*
